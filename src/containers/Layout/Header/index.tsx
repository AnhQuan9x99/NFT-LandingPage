...
const Header: React.FC<Props> = () => {
    ...
    const listMenu = [
        ...
        { name: 'Hot Collection', link: '/#collection' },
        { name: 'Art Icon Pet', link: '/#screenshot' },
        { name: 'App Feature', link: '/#features' },
        { name: 'Make NFT', link: '/#makenft' },
        { name: 'Faq', link: '/#faq' },
        ...
        <header id='header' className='sticky-top-slide'>
            <nav className='primary-menu navbar navbar-expand-lg fw 600 sticky-on'>
                <div className='container-lg'>
                    <Link href="/">
                        <a> <Image src='/../../../images/logo.png' className='header-logo' width ={125} height = {35}/></a>
                    </Link>
                        ...
                        <ul className='navbar-nav'>
                            {listMenu.map((menu, idx)=>(
                                <li className='nav-item' key={idx} onClick={()=> setPath(router.asPath)}>
                                    <Link href={menu.link}>
                                    <a className={`nav-link ${menu.link === path ? 'active' :''}`}>{menu.name}</a>
                                    </Link>
